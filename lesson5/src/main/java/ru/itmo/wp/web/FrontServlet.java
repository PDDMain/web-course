...
public class FrontServlet extends HttpServlet {
    ...
    private void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        if (isLang(request.getParameter("lang"))) {
            request.getSession().setAttribute("lang", request.getParameter("lang").toLowerCase());
        }
    ...
    private boolean isLang(String lang) {
        return lang != null && lang.length() == 2 && Pattern.compile("[a-zA-Z]{2}").matcher(lang).matches();
    }
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response) throws NotFoundException, ServletException, IOException {
        ...
        Method method = null;
        for (Class<?> clazz = pageClass; method == null; clazz = clazz.getSuperclass()) {
            ...
            for (Method m : methods) {
                ...
                if (m.getName().equals(route.action)) {
                    ...
                    if (flag && mapCount <= 1 && requestCount <= 1) {
                        method = m;
                        break;
                    }
                }
